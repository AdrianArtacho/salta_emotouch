...
proj_string = gui_enterstring.main(
    'Enter name defining the files', 'string', 'Project name',
    default_text='castadiva',
    verbose=False
)
...
json_path = "stats/"+proj_string+"-public-taps.json"
...
ax = sns.boxplot(x='series', y='gap_ms', data=long_df, palette='Set3')
plt.xticks(rotation=90)

# Set x-axis labels to last 5 characters of each series name
labels = [s[-5:] for s in long_df['series'].unique()]
...
plt.xlabel('')
plt.tight_layout()
plt.savefig("stats/stats_gaps_"+proj_string+".png", dpi=300, bbox_inches='tight')
plt.show()