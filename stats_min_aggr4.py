...
Q1 = df['taps_per_minute'].quantile(0.25)
Q3 = df['taps_per_minute'].quantile(0.75)
IQR = Q3 - Q1
df_filtered = df[(df['taps_per_minute'] >= Q1 - 1.5 * IQR) & (df['taps_per_minute'] <= Q3 + 1.5 * IQR)]

# Plot with secondary y-axis
fig, ax1 = plt.subplots(figsize=(6, 6))

# Primary axis: taps per minute
sns.boxplot(y='taps_per_minute', data=df_filtered, color='lightblue', ax=ax1)
ax1.set_ylabel('Taps per Minute')
ax1.set_xlabel('All Experiments Combined')
ax1.set_title('Taps per Minute with Equivalent Seconds per Tap (Outliers Removed)')
...
plt.tight_layout()
plt.savefig("stats/stats_tpm_and_seconds_filtered.png", dpi=300, bbox_inches='tight')
plt.show()
